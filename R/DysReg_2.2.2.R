...
DiffCor <- function(exp.1, exp.2, tf2tar, 
  ...
  for(i in 1 : length(tf.list)){
    diffreg.i <- cbind(cor.1[rownames(cor.1) == tf.list[i], ],
                       cor.2[rownames(cor.2) == tf.list[i], ])
    
    diffreg.i <- data.frame(TF = tf.list[i],Target = rownames(diffreg.i),
                            diffreg.i,stringsAsFactors = F)
    
    diffreg.i <- diffreg.i[diffreg.i$TF != diffreg.i$Target, ]
    
    colnames(diffreg.i)[3:4] <- c('cor.1','cor.2')
    
    diffreg.i$z.1 <- (0.5*log((1 + diffreg.i$cor.1)/(1 - diffreg.i$cor.1)))
    diffreg.i$z.2 <- (0.5*log((1 + diffreg.i$cor.2)/(1 - diffreg.i$cor.2)))
    
    diffreg.i$z.value <- (diffreg.i$z.2 - diffreg.i$z.1)/
      ((1/(ncol(exp.1)-3) + 1/(ncol(exp.2)-3))^0.5)
    
    diffreg.i$p.val <- 2 * (1 - pnorm(abs(diffreg.i$z.value)))
    diffreg.i$p.adj <- p.adjust(diffreg.i$p.val,method = p.adj)
    ...
    diffreg.res <- rbind(diffreg.res,diffreg.i)
  ...
  diffreg.res <- diffreg.res[order(abs(diffreg.res$z.value),decreasing = T),]
  rownames(diffreg.res) <- NULL
...
DiffCorPlus <- function(exp.1,exp.2, tf2tar,  
  ...
  diffreg.res <- DiffCor(exp.1, exp.2, tf2tar, cor.method = cor.method, p.adj = p.adj)

  diffreg.plus <- merge(diffreg.res,de.genes,by.x = 'Target',by.y = 'GeneSymbol')
  
  diffreg.plus <- diffreg.plus[,c(2,1,3:11)]
  diffreg.plus <- rbind(
                    subset(diffreg.plus, 
